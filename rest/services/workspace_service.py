...
class WorkspaceService(object):
    ...
    def patch_workspace(self, workspace_id: int, workspace_data: PatchWorkspaceRequest, auth_context: AuthorizationContextData):
        ...
        # Accept both new generic field and legacy github_access_token
        token_value = workspace_data.git_access_token or workspace_data.github_access_token
        if token_value:
            encrypted_secret = self.git_token_fernet.encrypt(
                data=token_value.encode('utf-8')
            )
            decoded_encrypted_secret = encrypted_secret.decode('utf-8')
        workspace.git_access_token = decoded_encrypted_secret
        workspace.github_access_token = decoded_encrypted_secret  # keep legacy in sync
        if workspace_data.git_platform_url:
            workspace.git_platform_url = workspace_data.git_platform_url
        if workspace_data.git_platform_type:
            workspace.git_platform_type = workspace_data.git_platform_type
        if workspace_data.git_username:
            workspace.git_username = workspace_data.git_username
        self.workspace_repository.update(workspace)
        token_filled = workspace.git_access_token is not None
        return PatchWorkspaceResponse(
            id=workspace.id,
            workspace_name=workspace.name,
            git_access_token_filled=token_filled,
            github_access_token_filled=token_filled,
            git_platform_type=workspace.git_platform_type,
            git_platform_url=workspace.git_platform_url,
    ...
    def list_active_pending_user_workspaces(self, page: int, page_size: int, auth_context: AuthorizationContextData) -> List[ListUserWorkspacesResponse]:
        ...
        response = [
            ListUserWorkspacesResponse(
                ...
                status=status,
                git_access_token_filled=(workspace.git_access_token or workspace.github_access_token) is not None,
                github_access_token_filled=(workspace.git_access_token or workspace.github_access_token) is not None
            )
    ...
    def get_workspace_data(self, workspace_id: int, auth_context: AuthorizationContextData) -> GetWorkspaceResponse:
        ...
        token_filled = (workspace.git_access_token or workspace.github_access_token) is not None
        response = GetWorkspaceResponse(
            ...
            git_access_token_filled=token_filled,
            github_access_token_filled=token_filled,
            git_platform_type=getattr(workspace, 'git_platform_type', None),
            git_platform_url=getattr(workspace, 'git_platform_url', None),
        )
        return response
    ...
    def handle_invite_action(self, workspace_id: int, auth_context: AuthorizationContextData, action: UserWorkspaceStatus):
        ...
        token_filled = (workspace.github_access_token or workspace.git_access_token) is not None
        response = GetWorkspaceResponse(
            id=workspace.id,
            workspace_name=workspace.name,
