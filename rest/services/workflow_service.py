...
class WorkflowService(object):
    def __init__(self) -> None:
        ...
        # Git client for remote DAG storage (optional, only used when DEPLOY_MODE != 'local-compose')
    ...
    def _get_git_client(self) -> GitPlatformClient:
        """Lazily build the remote git client for DAG storage."""
        if self._git_client is None:
            # Prefer generic git settings; fall back to legacy GitHub-specific settings
            repo_url = settings.DOMINO_WORKFLOWS_GIT_REPO_URL or ""
            token = settings.DOMINO_WORKFLOWS_GIT_TOKEN or settings.DOMINO_GITHUB_ACCESS_TOKEN_WORKFLOWS

            if not repo_url and settings.DOMINO_GITHUB_WORKFLOWS_REPOSITORY:
                # Legacy GitHub mode: repo path only (owner/repo)
                repo_url = f"https://github.com"
                self._git_client = GitPlatformClient(
    ...
    def _git_repo_path(self) -> str:
        """Return owner/repo path for the remote git DAG repository."""
        if settings.DOMINO_WORKFLOWS_GIT_REPO_URL:
            from urllib.parse import urlparse
            parsed = urlparse(settings.DOMINO_WORKFLOWS_GIT_REPO_URL)
            return parsed.path.lstrip("/").removesuffix(".git")
        return settings.DOMINO_GITHUB_WORKFLOWS_REPOSITORY

    def _git_branch(self) -> str:
        return settings.DOMINO_WORKFLOWS_GIT_REPO_BRANCH or "main"

    def _use_local_storage(self) -> bool:
        """True when DAGs should be written to the local filesystem."""
        return (
            settings.DOMINO_WORKFLOW_STORAGE_MODE == "local"
            or settings.DEPLOY_MODE == "local-compose"
        )
        ...
        try:
            ...
            if self._use_local_storage():
                ...
                git_client.create_file(
                    repo_path=repo_path,
                    file_path=f'workflows/{workflow_id}.py',
                    content=workflow_code,
                    branch=branch,
                )
    ...
    def get_workflow(self, workspace_id: int, workflow_id: str, auth_context: AuthorizationContextData) -> GetWorkflowResponse:
        ...
        # Use the version-safe helper that also handles 2.9+ timetable_summary
        schedule = AirflowRestClient.extract_schedule(airflow_dag_info)
        airflow_dag_info.pop("schedule_interval", None)
        airflow_dag_info.pop("timetable_summary", None)
    ...
    async def delete_workflow_files(self, workflow_uuid):
        ...
        try:
            ...
            self.logger.warning("Could not delete workflow file from git: %s", exc)
