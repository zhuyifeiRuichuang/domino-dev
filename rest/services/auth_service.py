...
class AuthService():
    ...
    git_token_fernet = Fernet(settings.GIT_TOKEN_SECRET_KEY)
    ...
    def workspace_access_authorizer(
        ...
        decoded_git_token = None
        raw_token = workspace_associative_data.git_access_token or workspace_associative_data.github_access_token
        if raw_token:
            decoded_git_token = cls.git_token_fernet.decrypt(raw_token.encode('utf-8')).decode('utf-8')
        auth_context.workspace = WorkspaceAuthorizerData(
            ...
            git_platform_url=getattr(workspace_associative_data, 'git_platform_url', None),
            git_platform_type=getattr(workspace_associative_data, 'git_platform_type', None),
            git_username=getattr(workspace_associative_data, 'git_username', None),
